...
// Implemented the quick sort logic over bid.title
...
int partition(vector<Bid>& bids, int begin, int end) {
	...
	while (!done) {
		...
		//keep incrementing low parameter as long as it's less than pivot
		while (bids.at(low).title.compare(bids.at(pivot).title) < 0) {
			++low;
		}

		// keep decrementing high parameter as long as it's less than pivot
